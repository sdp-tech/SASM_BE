...
class SNSUrl(models.Model):
    ...
    place = models.ForeignKey(
        "Place", related_name='place_sns_url', on_delete=models.CASCADE)
...
class Place(core_models.TimeStampedModel):
    ...
    place_likeuser_set = models.ManyToManyField(
        'users.User', related_name='PlaceLikeUser', blank=True)
    ...
    is_released = models.BooleanField(
        null=False, blank=False, default=True)  # 공개/심사중
...
class PlaceVisitorReviewCategory(core_models.TimeStampedModel):
    ...
    category_choice = models.ManyToManyField(
        "PlaceVisitorReview", related_name='category')
...
class PlaceVisitorReview(core_models.TimeStampedModel):
    # 방문자리뷰 모델은 Place 모델을 속성으로 가져야 함
...
class PlaceVisitorReviewPhoto(core_models.TimeStampedModel):
    imgfile = models.ImageField(
        null=True, upload_to=image_upload_path, blank=True)
    review = models.ForeignKey(
        "PlaceVisitorReview", related_name='photos', on_delete=models.CASCADE)

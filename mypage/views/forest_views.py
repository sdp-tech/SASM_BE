...
class OtherUserCreateForestApi(APIView):
    permission_classes = (IsAuthenticated, )
    ...
    class OtherUserCreatedForestFilterSerializer(serializers.Serializer):
        search = serializers.CharField(required=False)
        category_filter = serializers.ListField(required=False)

    class OtherUserCreatedForestOutputSerializer(serializers.Serializer):
        id = serializers.IntegerField()
        title = serializers.CharField()
        preview = serializers.SerializerMethodField()
        forest_like = serializers.SerializerMethodField()
    ...
    def get(self, request):
        filters_serializer = self.OtherUserCreatedForestFilterSerializer(
            data=request.query_params)
        filters_serializer.is_valid(raise_exception=True)
        filters = filters_serializer.validated_data
        ...
        selector = UserCreatedForestSelector(user=user)
        ...
        return get_paginated_response(
            pagination_class=self.Pagination,
            serializer_class=self.OtherUserCreatedForestOutputSerializer,
